...
def parse_args():
    ...
    parser.add_argument(
        "--apply_increlora",
        action="store_true",
        help="Whether to apply increlora or not.",
    )
...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir, now.strftime('%Y-%m-%d_%H:%M:%S') +"_"+ args.experiment_name)
    ...
    train_dataset = processed_datasets["train"] # 204045
    total_train_num = len(train_dataset)
    indices = list(range(0,total_train_num,5))
    ...
    if args.lora_type == "svd" and args.apply_increlora:
        rankallocator = RankAllocator(
            ...
            incre_interval=args.incre_interval, 
            top_h=args.top_h,
            beta1=args.beta1, 
            beta2=args.beta2, 
    ...
    time_file= os.path.join(args.output_dir, 'time.log')
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with open(time_file,"a") as f:
                f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_forward: {round(e-s,2)}s  \n")
            ...
            now = datetime.datetime.now()
            with open(time_file,"a") as f:
                f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Orth_regu: {round(e-s,2)}s  \n")
            ...
            now = datetime.datetime.now()
            with open(time_file,"a") as f:
                f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Backward: {round(e-s,2)}s  \n")
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                if rankallocator is not None:
                    ...
                    curr_rank, incre_threshold = rankallocator.update_and_increase(model, completed_steps, optimizer)
                #-----------------------------------------
                e = time.time()
                now = datetime.datetime.now()
                with open(time_file,"a") as f:
                    f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Allocation: {round(e-s,2)}s  \n")
                ...
                now = datetime.datetime.now()
                with open(time_file,"a") as f:
                    f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Update: {round(e-s,2)}s  \n")
        ...
        now = datetime.datetime.now()
        with open(time_file,"a") as f:
            f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Eval: {round(e-s,2)}s  \n")
        ...
        now = datetime.datetime.now()
        with open(time_file,"a") as f:
            f.write(f"{now.strftime('%Y-%m-%d_%H:%M:%S')}_Save: {round(e-s,2)}s  \n")
            
        with open(os.path.join(args.output_dir, "epoch_{}_all_results.json".format(epoch)), "w") as f:
            json.dump(
                {
