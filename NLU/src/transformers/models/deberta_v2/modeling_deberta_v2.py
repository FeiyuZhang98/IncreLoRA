        ...
        if config.apply_lora and "attention.output" in config.lora_module:
            if config.lora_type == "frd":
                ...
                self.dense = lora.SVDLinear(config.hidden_size, config.hidden_size, r=config.lora_r, 
                                            lora_alpha=config.lora_alpha, lora_dropout=config.lora_dropout, merge_weights=False)
            else:
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module.value_proj, 'lora_A'):
                params = module.value_proj.lora_A
                if isinstance(params, nn.ParameterList):
                    for p in params:
                        p.data.normal_(mean=0.0, std=self.config.initializer_range)
                else:
            ...
            if hasattr(module.query_proj, "lora_B") and self.config.lora_type == "svd":
                params = module.query_proj.lora_B
                if isinstance(params, nn.ParameterList):
                    for p in params:
                        p.data.normal_(mean=0.0, std=self.config.initializer_range)
                else:
            ...
            if hasattr(module.value_proj, "lora_B") and self.config.lora_type == "svd":
                params = module.value_proj.lora_B
                if isinstance(params, nn.ParameterList):
                    for p in params:
                        p.data.normal_(mean=0.0, std=self.config.initializer_range)
                else:
